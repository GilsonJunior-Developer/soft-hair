...
| 2.5-TEST-001 | low | P2 | open | E2E Playwright deferred (waiver Epic 2 herdado). **Pareado com 1.5/1.6/1.7/2.4-TEST-001** — setup único cobre 5 stories. Cobrir: lista→search→detail→notes-edit→soft-delete | @dev |
...
| 2.5-INFRA-001 | medium | P1 | open | GitHub Actions warns que `actions/checkout@v4`, `actions/setup-node@v4`, `pnpm/action-setup@v4` usam Node.js 20 deprecated. Forced-Node-24 starting **2026-06-02**. Atualizar workflows antes do deadline | @devops |
...
- 1.6-TEST-001 + 1.7-TEST-001 + 1.7-TEST-002 + **2.4-TEST-001** + **2.5-TEST-001** — setup único de Playwright + axe-core cobre as **5 stories** (Epic 1 e 2 inteiros) de uma vez. **Forte candidato a P0 antes de Epic 3** — Epic 2 fechou 100% sem nenhuma cobertura E2E, deixando a regressão indetectável em escala.
...
- **2.5-INFRA-001** — atualizar workflows GitHub Actions antes de **2026-06-02** (Node 20 → Node 24 forçado)
